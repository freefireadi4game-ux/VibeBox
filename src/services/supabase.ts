...
import { Song, Playlist, UserSettings } from '../types';
 ...
 */
export const formatSongForSupabase = (song: Song) => ({
  id: song.id,
...
export const formatPlaylistForSupabase = (playlist: Playlist) => ({
  id: playlist.id,
  name: playlist.name,
...
export async function upsertSongToCloud(song: Song): Promise<boolean> {
  ...
  try {
    const row = formatSongForSupabase(song);
...
export async function upsertPlaylistToCloud(playlist: Playlist): Promise<boolean> {
  const client = getSupabaseClient();
  if (!client) return false;

  try {
    const row = formatPlaylistForSupabase(playlist);
...
export async function bulkSyncToCloud(songs: Song[], playlists: Playlist[]): Promise<boolean> {
  const client = getSupabaseClient();
  if (!client) return false;

  try {
    if (songs.length > 0) {
      const songRows = songs.map(formatSongForSupabase);
    ...
    if (playlists.length > 0) {
      const playlistRows = playlists.map(formatPlaylistForSupabase);
