...
import { Search, Plus, Settings, Radio, Keyboard, Sparkles } from 'lucide-react';
...
export const TopHeader: React.FC<TopHeaderProps> = ({ onOpenShortcuts }) => {
  ...
  return (
    <header className="sticky top-0 z-20 w-full h-16 bg-[#06070a]/80 backdrop-blur-2xl border-b border-white/[0.06] px-4 sm:px-8 flex items-center justify-between select-none">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500 via-indigo-600 to-violet-700 flex items-center justify-center text-white shadow-md shadow-indigo-950/60 border border-white/20">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          {currentSong && isPlaying && (
            <span className="px-2.5 py-0.5 rounded-full bg-violet-500/10 border border-violet-500/25 text-violet-300 text-[10px] font-semibold flex items-center gap-1.5 backdrop-blur-md shadow-sm">
              <span className="w-1.5 h-1.5 rounded-full bg-violet-400 animate-pulse shadow-[0_0_6px_rgba(168,85,247,0.8)]" />
              <span>Playing</span>
            </span>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <button
          ...
          title="Search songs, artists (Press /)"
        >
          <Search className="w-3.5 h-3.5 text-zinc-400" />
          <span className="hidden sm:inline text-zinc-400 font-medium">Search tracks...</span>
          <kbd className="hidden lg:inline-block px-1.5 py-0.5 rounded bg-black/50 border border-white/10 text-[9px] font-mono text-zinc-400">
            /
          </kbd>
        </button>
        ...
        <button
          ...
          className="py-1.5 px-3 rounded-xl bg-gradient-to-r from-indigo-600 via-purple-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-semibold text-xs flex items-center gap-1.5 shadow-md shadow-indigo-950/50 border border-white/15 transition-all active:scale-95"
        >
